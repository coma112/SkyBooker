...
# igazából, a távolság alapján random beírok valamit
BASE_PRICES = {
    'short': (15000, 35000),
    'medium': (30000, 80000),
    'long': (70000, 250000)
...
# ugye ezek az esélyek magára a generálásra.
...
def estimate_flight_hours(dep_code, arr_code):
    # netről kinézek ilyen hülye kódokat xd
...
def main():
    ...
    try:
        ...
        print(f"{len(aircraft_ids)} aircraft generálva")

        print("Repterek")
        airport_id_map = {} # iata_code -> id
        ...
        for iata, name, city, country in AIRPORTS:
            ...
            # jó elfelejtettem lefuttatni a springet xd
        ...
        print(f"{len(airport_id_map)} reptér generálva")
        ...
        # úgy kellene hogy a kövi 90 napra generálunk ÉS még pár régebbi

        base_data = datetime.now() - timedelta(days=10)
        ...
        dep_time = base_data.replace(hour=dep_hour, minute=dep_minute, second=0, microsecond=0) + timedelta(days=dep_offset_days) 
        ...
        # a múltbeli járatok v ARRIVED v CANCELLED
        ...
        flight_ids.append((cur.fetchone()[0], aircraft_id, dep_time))

        print(f"{len(flight_ids)} járat generálva")

        # na hát, itt jön a gondom :///, szerintem nem tudnám jól legeneránli ezeket,
        # tehát beadom az AI-nak hogy én hogy képzeltem el, és ő megmutatja a képletet majd!

        print("Ülések")
        seat_count = 0

        aircraft_config = {}
        cur.execute("SELECT id, economy_seats, business_seats, first_seats, base_price FROM aircraft a JOIN(SELECT aircraft_id, base_price FROM flights) f ON a.id = f.aircraft_id LIMIT 1000")
        cur.execute("SELECT id, economy_seats, business_seats, first_seats FROM aircraft")

        for row in cur.fetchall():
            aircraft_config[row[0]] = {'eco': row[1], 'biz': row[2], 'first': row[3]}

        cur.execute("SELECT id, aircraft_id, base_price FROM flights")
        flight_info = {row[0]: {'aircraft_id': row[1], 'base_price': float(row[2])} for row in cur.fetchall()}

        for flight_id, aircraft_id, dep_time in flight_ids:
            info = flight_info.get(flight_id, {})
        ...
        print(f"{seat_count} ülés legenerálva")
        ...
        print(f"{len(passenger_ids)} utas generálva")

        print("Foglalások")
        booking_count = 0
        used_refs = set()

        cur.execute("SELECT id, flight_id, seat_class, price FROM seats WHERE is_available = true LIMIT 5000")
        available_seats = cur.fetchall() # (seatid, flightid, class, price)

        # 400 ülést lefoglalunk
        seats_to_book = random.sample(available_seats, min(400, len(available_seats)))

        for seat_id, flight_id, seat_class, seat_price in seats_to_book:
        ...
        print(f"{booking_count} foglalás generálva")
...
main()
# ezt majd megoldom h legyen flightbooking!!!
